...
public class AuthorityInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (it.hasNext()){
            ...
            //request这个参数的map里面的value是一个String[]
        ...
        if (user == null || (user.getRole().intValue() != Const.Role.ROLE_ADMIN)){
            ...
            // 将response托管到拦截器中，response中的相关属性就要重写
            ...
            response.setContentType("application/json;character=UFT-8"); // 这里要设置返回返回值的类型，因为全部是json接口
    ...
    // 所有处理完之后调用的，例如如果我们不是一个前后端分离的项目，返回一个modelandview的时候，是在视图呈现之后才调用的
    @Override
